...
import os
book = o.load_workbook('Опросник_для_инсталлляции_ПО_виртуализации_AccentOS_v2_4_2 (2).xlsx')
sheet = book['Сущности']
...
def create_json(min_r,max_r):
    for row in sheet.iter_rows(min_row=min_r, max_row=max_r, values_only=True):
        node_name = row[0].lower().replace(' ','_') if is_english(row[0]) else translit(row[0].lower().replace(' ','_'), 'ru', reversed=True)
        entity = {
            "name": node_name,
